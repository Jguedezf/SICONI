 ...
 * VERSIÓN: 10.1.0 (Fix Color Faltante)
 * DESCRIPCIÓN: Inventario con botones Luxury 3D y corrección de variables.
 * -----------------------------------------------------------------------------
 */
...
public class InventoryView extends JDialog {
    ...
    // --- PALETA DE COLORES (Aquí faltaba el Fucsia) ---
    ...
    private final Color COLOR_FUCSIA_NEON = new Color(255, 0, 127); // ¡AGREGADO!
    ...
    class AuditStockDialog extends JDialog {
        ...
        public AuditStockDialog(JDialog parent, int id, String nombre, int stockActual) {
            ...
            header.setOpaque(false); header.setBorder(new EmptyBorder(20,20,10,20));
            JLabel l1 = new JLabel(nombre.toUpperCase(), SwingConstants.CENTER);
            l1.setFont(new Font("Segoe UI", Font.BOLD, 20)); l1.setForeground(COLOR_GOLD);
            JLabel l2 = new JLabel("ACTUAL: " + stockActual, SwingConstants.CENTER);
            l2.setForeground(Color.LIGHT_GRAY);
            header.add(l1); header.add(l2);
            add(header, BorderLayout.NORTH);

            JPanel center = new JPanel(new GridLayout(4,1,10,10));
            center.setOpaque(false); center.setBorder(new EmptyBorder(10,40,10,40));

            // BOTONES DE ENTRADA/SALIDA
            JPanel btns = new JPanel(new GridLayout(1, 2, 15, 0)); btns.setOpaque(false);
            btnIn = new LuxuryOptionBtn("ENTRADA", COLOR_VERDE_NEON);
            btnOut = new LuxuryOptionBtn("SALIDA", COLOR_GRIS_BTN);
            btnIn.setSelected(true);

            btnIn.addActionListener(e -> { esEntrada=true; updateBtnState(); calc(stockActual); });
            btnOut.addActionListener(e -> { esEntrada=false; updateBtnState(); calc(stockActual); });
            btns.add(btnIn); btns.add(btnOut);
            center.add(btns);

            txtCantidad = new JTextField();
            txtCantidad.setFont(new Font("Segoe UI", Font.BOLD, 36));
            txtCantidad.setHorizontalAlignment(SwingConstants.CENTER);
            txtCantidad.setBackground(new Color(40,40,40));
            txtCantidad.setForeground(Color.WHITE);
            txtCantidad.setCaretColor(COLOR_GOLD);
            txtCantidad.setBorder(BorderFactory.createLineBorder(Color.DARK_GRAY));
            ...
            lblResultado.setForeground(COLOR_GOLD);
            lblResultado.setFont(new Font("Segoe UI", Font.BOLD, 14));
            center.add(lblResultado);

            txtObs = new JTextArea();
            txtObs.setBackground(new Color(40,40,40));
            txtObs.setForeground(Color.LIGHT_GRAY);
            txtObs.setCaretColor(COLOR_GOLD);
            JScrollPane spObs = new JScrollPane(txtObs);
            spObs.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.GRAY), "MOTIVO / OBSERVACIÓN", 0,0, new Font("Segoe UI", Font.PLAIN, 10), Color.GRAY));
            center.add(spObs);
            add(center, BorderLayout.CENTER);

            JButton btnSave = new JButton("GUARDAR CAMBIOS");
            btnSave.setBackground(COLOR_GOLD); btnSave.setForeground(Color.BLACK);
            ...
            btnSave.addActionListener(e -> {
                try {
                    ...
                    if(c > 0) {
                        productDAO.auditStock(id, delta, txtObs.getText());
                        dispose();
                    } else JOptionPane.showMessageDialog(this, "Cantidad debe ser mayor a 0");
        ...
        private void updateBtnState() {
            btnIn.setSelected(esEntrada);
            btnOut.setSelected(!esEntrada);
        }
        private void calc(int act) {
            try { int n = Integer.parseInt(txtCantidad.getText()); lblResultado.setText("PROYECCIÓN: " + (esEntrada ? act+n : act-n)); }
            catch(Exception e) {}
        }
    }

    // CLASE PARA BOTONES REDONDEADOS
    class LuxuryOptionBtn extends JButton {
        private Color baseColor;
        private boolean isSelected;

        public LuxuryOptionBtn(String text, Color bg) {
            super(text);
            this.baseColor = bg;
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            if (isSelected) {
                ...
                g2.setColor(new Color(60, 60, 60)); // Desactivado
                setForeground(Color.GRAY);
            }
            g2.fillRoundRect(0, 0, getWidth(), getHeight(), 30, 30); // Borde redondeado
